...
| `title` | `NULL` | Written into the YAML of `manuscript.qmd` and `title_page.qmd`. Quotes and backslashes are escaped, so a LaTeX fragment such as `\textit{Formica}` survives. Left out, the placeholder stays and `make.R` warns until you replace it |
| `authors` | `NULL` | The names in order, as a vector or one comma-separated string. The first is marked corresponding. Affiliations are not guessed: they go in `_sections/0_authors.qmd` |
...
#> Project created: /tmp/RtmpEFNw1q/demo_paper
...
| `also` | none | Extensions to treat as already open for this call, with or without the dot. Permanently: add them to `.cd_open_formats` in the project’s `R/convert_data.R` |
...
recomputing it — and `clean_cache()` empties that cache, which is what
you run when the data underneath a chunk changed, because a change in
`data/` is the one thing knitr’s cache does not notice.
...
`_sections/6_figures.qmd`: move the chunk and every reference in the
...
Figures are written to `figures/png/` at 600 dpi on every render, and
`export_figure_formats()` leaves a `.jpg` and a `.tiff` copy beside each
one — which is what a journal asks for on acceptance, and what
`make_submission()` renumbers into `Figure_1.tiff` in the order the
reader meets them.

Supplementary figures and tables are the same chunks with `sfig-` and
`stbl-` labels, in `_sections/8*suppl*.qmd`, on a counter of their own.
They have a section further down.
...
make_all()                          # docx, pdf, supplement and code, in order
```
...
Three more exist for the supplement alone. `render_supplementary()`
takes `output_format` (`"docx"` by default, `"pdf"` for a preprint
deposit), `files`, a subset of the supplementary documents — which is
how `make_submission()` renders only the supplementary *text* when the
figures are staying in the main document — and `blinded` (`FALSE` by
default), which drops the authors from the supplement the way a
double-blind submission needs.
